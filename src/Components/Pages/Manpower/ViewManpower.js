...
import Popup from "./ManpowerModal";
...
const DataTable = () => {
  ...
  const removeManPower = (mpId) => {
    axios
      .post("http://entemadb.entema-software.com/removeMPDataonId", {
        MPID: mpId,
      })
      .then((res) => {
        ...
        >
          <div className="heading-layout1">
            <div className="item-title">
              <h3 style={{ padding: "50px" }}>List of Manpower</h3>
            </div>
          </div>
                  ...
                  {commentsData.map((comment) => (
                    <tr>
                      <th
                        scope="row"
                        key={comment.MP_ID}
                        onClick={() => openInPopup(comment.MP_ID)}
                        style={{ cursor: "pointer" }}
                      >
                        {comment.MP_ID}
                      </th>
                      <td o>{comment.MP_IQAMA_ID}</td>
                      <td>{comment.MP_NAME}</td>
